...
html_semantic_tags = ["<html>", "<head>", "<body>", "</html>", "</head>", "</body>"]
...
def count_tokens(tokens):
    ...
    return {"keywords": keywords, "identifiers": identifiers, "symbols": symbols, "strings": strings, "numbers": numbers}
...
def sintactic_analysis(tokens):
    ...
    for i, token in enumerate(tokens):
        ...
        if last_token and last_token['type'] == 'keyword' and last_token['value'] == 'echo':
            ...
            elif token['type'] != 'symbol' or token['value'] != ';':
                errors.append(f"Error en línea {last_token['line']}: Falta ';' después de '{last_token['value']}'")
...
def index():
    ...
    if request.method == 'POST':
        ...
        semantico_result = semantic_analysis(codigo)
        ...
        total_numbers = token_count["numbers"]

        return render_template('home.html', tokens=tokens, total_tokens=total_tokens, total_keywords=total_keywords, total_identifiers=total_identifiers, total_symbols=total_symbols, total_strings=total_strings, total_numbers=total_numbers, codigo=codigo, sintactico_result=sintactico_result, semantico_result=semantico_result)
